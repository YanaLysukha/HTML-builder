...
fs.readFile(templateFile, 'utf8', (error, data) => {
  ...
  fs.readdir(componentsDirectory, { withFileTypes: true }, (error, componentFiles) => {
    if (error) return console.error(error.message);
    const filteredComponents = componentFiles.filter((file) => path.extname(file.name) === '.html');

    for (const component of filteredComponents) {
      fs.readFile(path.join(componentsDirectory, component.name), 'utf-8', (error, componentData) => {
        if (error) return console.error(error.message);
